...
```bash
# Install the skill in your OpenClaw agent workspace
mkdir -p .claude/skills/forgetting-mechanism
cp SKILL.md .claude/skills/forgetting-mechanism/
```
...
```javascript
// Example OpenClaw agent configuration with forgetting mechanism
const agentConfig = {
  // ... other configuration
  forgettingMechanism: {
    strategies: [
...
const { OpenClawForgettingMechanism, MemoryStore } = require('./index');

// Create memory store with sample memories
const memoryStore = new MemoryStore([
  {
    id: "mem-1",
    content: "Project meeting notes from January",
...
console.log("Remaining memories:", memoryStore.getAll().length);
```
...
forgetting.forgetSpecificMemory("mem-123");

// Forget all memories related to a specific topic
forgetting.forgetByTopic("confidential");

// Forget all memories from a specific source
forgetting.forgetBySource("temporary");
```
...
const dryRunResult = forgetting.dryRun();
